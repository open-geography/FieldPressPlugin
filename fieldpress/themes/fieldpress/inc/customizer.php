 ...
 */
function fieldpress_customize_register( $wp_customize ) {
    ...
    $wp_customize->add_setting( 'fieldpress_logo', array(
    ...
    $wp_customize->add_control(
        ...
        'settings' => 'fieldpress_logo',
            )
    ) );

    if ( $wp_customize->is_preview() && !is_admin() ) {
        add_action( 'wp_footer', 'fieldpress_customize_preview', 21 );
    }
}

function fieldpress_customize_preview() {
    ?>
    <script type="text/javascript">
        ( function( $ ) {
            wp.customize( 'fieldpress_logo', function( value ) {
                value.bind( function( to ) {
                    $( "#fieldpress_logo" ).attr( "src", to ? to : '' );
                } );
            } );
        } )( jQuery )
    </script>
...
add_action( 'customize_register', 'fieldpress_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function fieldpress_customize_preview_js() {
    wp_enqueue_script( 'fieldpress_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20130508', true );
}

add_action( 'customize_preview_init', 'fieldpress_customize_preview_js' );