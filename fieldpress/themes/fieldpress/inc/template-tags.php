 ...
 * @package FieldPress
 */
if ( !function_exists( 'fieldpress_paging_nav' ) ) :
	 ...
	 * @return void
	 */
	function fieldpress_paging_nav() {
...
if ( !function_exists( 'fieldpress_post_nav' ) ) :
...
if ( !function_exists( 'fieldpress_comment' ) ) :
	 ...
	 */
	function fieldpress_comment( $comment, $args, $depth ) {
	...
	endif; // ends check for fieldpress_comment()

	if ( !function_exists( 'fieldpress_discussion_comment' ) ) :
		 ...
		 * Used as a callback by wp_list_comments() for displaying the comments.
		 */
		function fieldpress_discussion_comment( $comment, $args, $depth ) {
			$GLOBALS[ 'comment' ] = $comment;
	...
	if ( !function_exists( 'fieldpress_posted_on' ) ) :

		/**
		 * Prints HTML with meta information for the current post-date/time and author.
		 */
		function fieldpress_posted_on() {
	 ...
	 */
	function fieldpress_categorized_blog() {
		...
		if ( '1' != $all_the_cool_cats ) {
			// This blog has more than 1 category so fieldpress_categorized_blog should return true.
			return true;
		} else {
			// This blog has only 1 category so fieldpress_categorized_blog should return false.
			return false;
		}
	}

	/**
	 * Flush out the transients used in fieldpress_categorized_blog.
	 */
	function fieldpress_category_transient_flusher() {
		// Like, beat it. Dig?
		delete_transient( 'all_the_cool_cats' );
	}

	add_action( 'edit_category', 'fieldpress_category_transient_flusher' );
	add_action( 'save_post', 'fieldpress_category_transient_flusher' );
