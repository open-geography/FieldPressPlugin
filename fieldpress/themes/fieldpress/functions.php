<?php
/**
 * FieldPress functions and definitions
 *
 * @package FieldPress
 */
...
if ( !function_exists( 'fieldpress_setup' ) ) :
	 ...
	 */
	function fieldpress_setup() {

		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on FieldPress, use a find and replace
		 * to change 'fieldpress' to the name of your theme in all the template files
		 */
		...
		add_theme_support( 'custom-background', apply_filters( 'fieldpress_custom_background_args', array(
			'default-color'	 => 'f9f9f9',
			'default-image'	 => '',
		) ) );

		/* add_theme_support( 'custom-header', apply_filters( 'fieldpress_custom_header_args', array(
...
endif; // fieldpress_setup
add_action( 'after_setup_theme', 'fieldpress_setup' );

/**
 * Register widgetized area and update sidebar with default widgets.
 */
function fieldpress_widgets_init() {
...
add_action( 'widgets_init', 'fieldpress_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function fieldpress_scripts() {
	global $post;

	wp_enqueue_style( 'fieldpress-style', get_stylesheet_uri() );

	wp_enqueue_style( 'fieldpress-responsive-navigation', get_template_directory_uri() . '/css/responsive-nav.css' );

	wp_enqueue_script( 'fieldpress-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20120206', true );

	wp_enqueue_script( 'fieldpress-responsive-navigation', get_template_directory_uri() . '/js/responsive-nav.min.js', array(), '20120206', true );

	wp_enqueue_script( 'fieldpress-general', get_template_directory_uri() . '/js/script.js', array(), '20120207', true );

	wp_enqueue_script( 'fieldpress-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );
...
add_action( 'wp_enqueue_scripts', 'fieldpress_scripts' );
...
function cp_filter_search( $query ) {
	if ( $query->is_search ) {
		if ( !is_admin() ) {
			$query->set( 'post_type', array( 'post', 'field' ) );
		}
	}
	return $query;
}
...
add_shortcode( 'contact_form', 'fieldpress_contact_form' );

function fieldpress_contact_form() {
	ob_start();
...
/* Add thickbox to all images on Stop Elements */
...
/* Add thickbox to all images on Stop Single pages */

add_filter( 'the_content', 'stop_content' );

function stop_content( $content ) {
	if ( get_post_type( $GLOBALS[ 'post' ]->ID ) == 'stop' ) {
