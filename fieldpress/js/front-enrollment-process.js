function fieldpress_apply_data_link_click() {
...
jQuery( document ).ready( function( $ ) {
    ...
    $( 'button.apply-button.enroll' ).click( function( e ) {
        e.preventDefault();
        e.stopPropagation();
        open_popup( 'enrollment', $( this ).attr( 'data-field-id' ) );
    } );
    ...
    $( document.body ).on( 'click', '.cp_login_step', function( e ) {
        e.preventDefault();
        e.stopPropagation();
        open_popup( 'login', $( this ).attr( 'data-field-id' ) );
    } );
                        ...
                        if ( $( "#cp_popup_passcode" ).length > 0 ) {
                            $.post(
                                cp_vars.admin_ajax_url, {
                                    ...
                                    field_id: $( '.cp_signup_step' ).attr( 'data-field-id' )
                                }
                            ).done( function( data, status ) {
                                if ( status == 'success' ) {
                                    if ( data == 'valid' ) {
                                        ...
                                        open_popup( step, $( '.apply-button.login' ).attr( 'data-field-id' ) );
                                    } else {
                                        errors++;
                                                    ...
                                                    if ( $( "#cp_popup_passcode" ).length > 0 ) {
                                                        ...
                                                        $.post(
                                                            cp_vars.admin_ajax_url, {
                                                                action: 'cp_valid_passcode',
                                                                passcode: $( '#cp_popup_passcode' ).val(),
                                                                field_id: $( '.signup-data' ).attr( 'data-field-id' )
                                                            }
                                                        ).done( function( data, status ) {
                                                            if ( status == 'success' ) {
                                                                if ( data == 'valid' ) {
                                                                    ...
                                                                    open_popup( step, $( '#data-field-id' ).attr( 'data-field-id' ), $( '#popup_signup_form' ).serialize() );
                                                                } else {
                                                                    errors++;
                                                                    $( '#popup_login_form .validation_errors' ).html( cp_vars.message_passcode_invalid );
                                                                    validate_mark_error_field( 'cp_popup_passcode' );
    ...
    function prepare_payment_data_and_submit( element ) {
        ...
        cp_popup_load_content( 'process_payment', field_id, field_data );
    ...
    function open_popup( step, field_id, data ) {
        if ( typeof data === 'undefined' ) {//data not set
            content_loaded = cp_popup_load_content( step, field_id );
        } else {
            content_loaded = cp_popup_load_content( step, field_id, data );
        }
    ...
    function cp_popup_load_content( step, field_id, data ) {
