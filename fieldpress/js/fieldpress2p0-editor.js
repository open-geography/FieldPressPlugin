...
(function ( $ ) {
    ...
    FieldPress.Events = FieldPress.Events || _.extend( {}, Backbone.Events );

    FieldPress.editor = FieldPress.editor || {};

    if ( typeof getUserSetting !== 'undefined' ) {
        FieldPress.editor.init_mode = getUserSetting( 'editor' );
    }

    FieldPress.editor.create = function ( target, id, name, content, append, height ) {
        ...
        var editor = _fieldpress._dummy_editor;
        ...
        if ( undefined !== options ) {
            ...
            options.init_instance_callback = 'FieldPress.editor.on_init'; // code to execute after editor is created
            options.cache_suffix = '';
            options.setup = function ( ed ) {
                ed.on( 'keyup', function ( arr ) {
                    var content = FieldPress.editor.content( id );
                    $( '#' + id ).html( content );

                    FieldPress.Events.trigger( 'editor:keyup', ed );
                } );
            };
    ...
    FieldPress.editor.content = function ( id, content ) {
    ...
    FieldPress.editor.set_height = function ( id, height ) {
        $( '#wp-' + id + '-editor-container' ).removeAttr( 'rows' );
        $( '#wp-' + id + '-wrap iframe' ).css( 'height', height + 'px' )
    }

    FieldPress.editor.on_init = function ( instance ) {

        var mode = FieldPress.editor.init_mode;
        ...
        var mce_button = '<button id="' + instance.id + '-visual' + '" class="wp-switch-editor switch-tmce" type="button">' + _fieldpress.editor_visual + '</button>';
        var qt_button = '<button id="' + instance.id + '-text' + '" class="wp-switch-editor switch-html" type="button">' + _fieldpress.editor_text + '</button>';
        ...
        FieldPress.Events.trigger( 'editor:created', instance );
    }

    FieldPress.editor.init = function( selector ) {

        selector = selector || '.fieldpress-editor';
