...
if ( ! class_exists( 'CP_Certificate_Templates' ) ) {
	...
	class CP_Certificate_Templates {
		...
		function generate_certificate( $field_id = false, $user_id = false, $preview = false, $force_download = false ) {
			global $cp, $pdf;
			...
			$template_id = get_post_meta( $field->details->ID, 'certificate_template', true );
			...
			$pdf = new FieldPress_PDF( $metas['document_template_orientation'], PDF_STOP, apply_filters( 'fieldpress_additional_template_document_size_output', $metas['document_template_size'] ), true, apply_filters( 'fieldpress_template_document_encoding', bloginfo( 'charset' ) ), false );
			...
			for ( $i = 1; $i <= apply_filters( 'fieldpress_template_template_row_number', 10 ); $i ++ ) {
					...
					foreach ( $element_class_names as $element_class_name ) {
						...
						if ( class_exists( $element_class_name ) ) {
							...
							if ( isset( $post_id ) ) {
								...
								$rows .= $element->template_content( $field_id, $user_id, $preview );
			...
			$pdf->lastPage();
			$pdf->Output( ( isset( $field->details->post_name ) ? $field->details->post_name : __( 'Certificate', 'cp' ) ) . '.pdf', ( $force_download ? 'D' : 'I' ) );
			exit;
		}
		...
		function add_new_template() {
			...
			if ( isset( $_POST['template_title'] ) ) {
				...
				$post = apply_filters( 'fieldpress_certificate_template_post', $post );
				...
				if ( $post_id != 0 ) {
					foreach ( $_POST as $key => $value ) {
						if ( preg_match( "/_post_meta/i", $key ) ) {//every field name with sufix "_post_meta" will be saved as post meta automatically
							...
							do_action( 'fieldpress_template_post_metas' );
						}
					}
				}
		...
		function get_template_col_fields() {
			...
			return apply_filters( 'fieldpress_template_col_fields', $default_fields );
		}
