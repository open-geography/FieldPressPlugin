...
include_once( FieldPress::instance()->plugin_dir . 'includes/external/tcpdf/config/lang/eng.php' );
require_once( FieldPress::instance()->plugin_dir . 'includes/external/tcpdf/tcpdf.php' );
 ...
 * @return object
 */
class FieldPress_PDF extends TCPDF {
	...
	function make_pdf( $html, $args = array() ) {
		...
		if( ! isset( $args['title'] ) || empty( $args['title'] ) ) {
			$args['title'] = __( 'FieldPress Report', 'cp' );
		}
		...
		$pdf = new TCPDF( $page_orientation, PDF_STOP, PDF_PAGE_FORMAT, true, 'UTF-8', false );
		...
		// $pdf = new TCPDF( $page_orientation, PDF_STOP, PDF_PAGE_FORMAT, false, 'ISO-639-2', false );

		// set document information
		$pdf->SetCreator( FieldPress::instance()->name );
		...
		$furl = FieldPress::instance()->plugin_url . 'includes/external/tcpdf/cache/' . $uid . '.pdf';
