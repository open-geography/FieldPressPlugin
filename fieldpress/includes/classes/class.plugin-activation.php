	...
	class CP_Plugin_Activation {
		...
		public function __construct() {
			...
			$this->plugins['marketpress'] = array(
					...
					//'source'         => FieldPress::instance()->plugin_dir . 'includes/plugins/' . FieldPress::instance()->mp_file,
					'source'         => is_object( $fieldpress ) ? $fieldpress->plugin_dir . 'includes/plugins/' . $fieldpress->mp_file : '',
					// The plugin source.
			...
			$this->strings = array(
				...
				'notice_can_install_recommended'  => sprintf( __( 'Install %1$s plugin in order to sell field trips.', 'cp' ), $this->plugin['name'] ),
				'notice_can_activate_recommended' => '',
				'install_link'                    => sprintf( __( 'Install %1$s', 'cp' ), $this->plugin['name'] ),
				'activate_link'                   => sprintf( __( 'Activate %1$s plugin in order to sell field trips', 'cp' ), $this->plugin['name'] ),
