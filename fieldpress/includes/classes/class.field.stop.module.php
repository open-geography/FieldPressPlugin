<?php
if ( ! class_exists( 'Stop_Module' ) ) {

	class Stop_Module extends FieldPress_Object {
		...
		function update_module( $data ) {
			...
			if( empty($data->stop_id) || $data->stop_id == 0) return;

			$post = array(
				'post_author'  => $user_id,
				'post_parent'  => $data->stop_id,
			...
			self::kill( self::TYPE_STOP_MODULES, $data->stop_id );
			self::kill( self::TYPE_STOP_MODULES_PERF, get_post_field( 'post_parent', $data->stop_id ) );
			...
			if ( isset( $data->metas ) ) {
				...
				if ( in_array( $module_type, $input_module_types ) ) {
					...
					Stop::update_input_module_meta( $stop_id, $module_id, $module_meta );
				}
			}
			...
			if ( $new_module ) {
				 ...
				 * @since 1.2.2
				 */
				do_action( 'fieldpress_stop_module_created', $post_id, $data->stop_id );
			} else {
				 ...
				 * @since 1.2.2
				 */
				do_action( 'fieldpress_stop_module_updated', $post_id, $data->stop_id );
			}
		...
		public static function delete_module( $id, $force_delete = true ) {
			...
			$stop_id = self::get_module_stop_id( $id );

			/**
			 * Allow Stop Module deletion to be cancelled when filter returns true.
			 *
			 * @since 1.2.2
			 */
			if ( apply_filters( 'fieldpress_stop_module_cancel_delete', false, $id, $stop_id ) ) {
				 ...
				 * @since 1.2.2
				 */
				do_action( 'fieldpress_stop_module_delete_cancelled', $id, $stop_id );
			...
			self::kill( self::TYPE_STOP_MODULES, $stop_id );
			self::kill( self::TYPE_STOP_MODULES_PERF, get_post_field( 'post_parent', $stop_id ) );

			if ( get_post_type( $id ) == 'module' ) {
				wp_delete_post( $id, $force_delete ); //Whether to bypass trash and force deletion
			}
			//Delete stop module responses
			...
			$stops_module_responses = get_posts( $args );

			foreach ( $stops_module_responses as $stops_module_response ) {
				if ( get_post_type( $stops_module_response->ID ) == 'module_response' ) {
					wp_delete_post( $stops_module_response->ID, true );
				}
			}

			// Remove input module meta
			Stop::delete_input_module_meta( $stop_id, $id );

			/**
			 * Perform actions after a Stop Module is deleted.
			 *
			 * @var $the_module  The Stop Module object
			 *
			 * @since 1.2.2
			 */
			do_action( 'fieldpress_stop_module_deleted', $the_module, $stop_id );
		}
			...
			if ( is_admin() ) {
				if ( isset( $_POST['modules_to_execute'] ) ) {
					...
					foreach ( $modules_to_delete as $module_to_delete ) {
						...
						Stop_Module::delete_module( $module_to_delete, true );
		...
		public static function did_student_respond( $stop_module_id, $student_id ) {
		...
		function update_module_response( $data ) {
			global $user_id, $wpdb, $fieldpress;

			$user_id   = get_current_user_id();
			$stop_id   = get_post_ancestors( $data->response_id );
			$field_id = get_post_meta( $stop_id[0], 'field_id', true );
			...
			if ( count( $already_respond_posts ) == 0 ) {
				...
				if ( isset( $data->auto_grade ) && is_numeric( $data->auto_grade ) ) {
					Stop_Module::save_response_grade( $post_id, $data->auto_grade, get_current_user_id(), $field_id, $stop_id[0], $data->module_id );
					do_action( 'student_response_not_required_grade_instructor_notification', get_current_user_id(), $field_id, $instructors );
				} else {
					do_action( 'student_response_required_grade_instructor_notification', get_current_user_id(), $field_id, $instructors );
				}

				//$fieldpress->set_latest_activity( get_current_user_id() );
				return $post_id;
			} else {
				return false;
			}
		...
		public static function get_module_stop_id( $module_id ) {
			global $post;
		...
		public static function get_modules( $stop_id, $stop_page = 0, $ids_only = false ) {

			$stop_pagination = cp_stop_uses_new_pagination( (int) $stop_id );
			...
			$new_request = ! ( isset( self::$last_modules_request['stop_id'] ) && self::$last_modules_request['stop_id'] == $stop_id && self::$last_modules_request['stop_page'] == $stop_page && self::$last_modules_request['ids_only'] == $ids_only && ! empty( self::$last_modules_request['modules'] ) );

			// Attempt to load from cache or create new cache object
			if ( $ids_only ) {
				$cache_id = $stop_id . '-' . $stop_page . '-ids';
			} else {
				$cache_id = $stop_id . '-' . $stop_page;
			}
			if ( ! self::load( self::TYPE_STOP_MODULES, $cache_id, $modules ) && $new_request ) {

				// Get the modules
				if ( $stop_pagination && $stop_page > 0 ) {
				...
				// Cache the field trip object
				self::cache( self::TYPE_STOP_MODULES, $cache_id, $modules );

				// cp_write_log( 'Stop Modules[' . $stop_id . ']: Saved to cache..');
			} else {
			...
			self::$last_modules_request['stop_id'] = $stop_id;
			self::$last_modules_request['stop_page'] = $stop_page;
		...
		function get_modules_admin_forms( $stop_id = 0 ) {
			global $fieldpress_modules;

			$modules = self::get_modules( $stop_id );
		...
		public static function get_modules_front( $stop_id = 0 ) {
			global $fieldpress, $fieldpress_modules, $wp, $paged, $_POST;
			...
			if ( isset( $_GET['resubmit_nonce'] ) || ( isset( $_GET['resubmit_nonce'] ) && wp_verify_nonce( $_GET['resubmit_nonce'], 'resubmit_answer' ) ) ) {
				if ( isset( $_GET['resubmit_answer'] ) ) {
					...
					Student_Completion::clear_mandatory_answer( $user_id, $field_id, $stop_id, $module_id );
					...
					if ( isset( $response ) && isset( $response->post_author ) && $response->post_author == get_current_user_ID() ) {
						...
						Student_Completion::record_mandatory_answer( $user_id, $field_id, $stop_id, $module_id );
			...
			$stop_pagination = cp_stop_uses_new_pagination( (int) $stop_id );
			$modules         = self::get_modules( $stop_id, $paged );
			 ...
			 */
			cp_set_visited_stop_page( $stop_id, $paged, get_current_user_ID(), $field_id );

			//$stop_module_page_number = isset( $_GET['to_elements_page'] ) ? $_GET['to_elements_page'] : 1;
			...
			if ( isset( $_POST['submit_modules_data_done'] ) || isset( $_POST['submit_modules_data_no_save_done'] ) ) {
				...
				if ( isset( $_POST['submit_modules_data_done'] ) ) {
					...
					if ( $_POST['event_origin'] == 'button' ) {
						wp_redirect( get_permalink( $field_id ) . trailingslashit( $fieldpress->get_stops_slug() ) . '?saved=ok' );
						exit;
					} else {
						wp_redirect( cp_full_url( $_SERVER ) ) . '?saved=ok';
						exit;
					...
					if ( $_POST['event_origin'] == 'button' ) {
						wp_redirect( trailingslashit( get_permalink( $field_id ) ) . trailingslashit( $fieldpress->get_stops_slug() ) );
						exit;
					} else {
					...
					/* if ( $paged != 1 ) {
					  //wp_redirect( cp_full_url( $_SERVER ) );
					  wp_redirect(get_permalink($field_id) . trailingslashit($fieldpress->get_stops_slug()));
					  } else {
					  wp_redirect(cp_full_url($_SERVER));
					  } */
				}
			...
			if ( isset( $_POST['submit_modules_data_save'] ) || isset( $_POST['submit_modules_data_no_save_save'] ) ) {
				...
				if ( isset( $_POST['submit_modules_data_save'] ) ) {
					...
					//wp_redirect( get_permalink( $stop_id ) . trailingslashit( 'page' ) . trailingslashit( $stop_module_page_number ) );
				}
			}

			if ( isset( $_POST['save_student_progress_indication'] ) ) {
				wp_redirect( get_permalink( $field_id ) . trailingslashit( $fieldpress->get_stops_slug() ) . '?saved=progress_ok' );
				exit;
			}
			?>

			<form name="modules_form" id="modules_form" enctype="multipart/form-data" method="post" action="<?php echo trailingslashit( get_permalink( $stop_id ) ); //strtok( $_SERVER["REQUEST_URI"], '?' ); ?>" onSubmit="return check_for_mandatory_answers();">
				<!--#submit_bottom-->
				...
				if ( $stop_pagination ) {
					$pages_num = fieldpress_stop_pages( $stop_id, $stop_pagination );
				}

				$is_last_page = fieldpress_stop_module_pagination( $stop_id, $pages_num, true ); //check if current stop page is last page
				if ( ! $fieldpress->is_preview( $stop_id ) ) {
					if ( $front_save ) {
						if ( $input_modules !== $responses ) {
							...
							<input type="hidden" name="stop_id" value="<?php echo $stop_id; ?>"/>
							<a id="submit_bottom"></a>
							<?php
							if ( isset( $_POST['submit_modules_data'] ) ) {
								$form_message = __( 'The module data has been submitted successfully.', 'fieldpress' );
							}
				...
				fieldpress_stop_module_pagination( $stop_id, $pages_num );
				?>
		...
		function additional_module_actions() {
			...
			Stop_Module::save_response_grade();
		}
		...
		public static function save_response_grade( $response_id = '', $response_grade = '', $user_id = false, $field_id = false, $stop_id = false, $module_id = false ) {
			if ( ( isset( $_POST['response_id'] ) || $response_id !== '' ) && ( isset( $_POST['response_grade'] ) || $response_grade !== '' ) ) {
				...
				if ( ! $user_id ) {
					...
					$stop_id   = isset( $_POST['stop_id'] ) ? (int) $_POST['stop_id'] : false;
					$module_id = isset( $_POST['module_id'] ) ? (int) $_POST['module_id'] : false;
				}

				if ( ! empty( $user_id ) && ! empty( $field_id ) && ! empty( $stop_id ) && ! empty( $module_id ) ) {
					$mandatory_answer = get_post_meta( $module_id, 'mandatory_answer', true );
					if ( ! empty( $mandatory_answer ) && 'yes' == $mandatory_answer ) {
						Student_Completion::record_mandatory_answer( $user_id, $field_id, $stop_id, $module_id );
					}
					Student_Completion::record_gradable_result( $user_id, $field_id, $stop_id, $module_id, floatval( $grade_data['grade'] ) );
				}
		...
		public static function get_response_grade( $response_id, $data = '' ) {
			...
			$autograde_modules = Stop_Module::auto_grade_modules();
			...
			if ( in_array( $module_type, $autograde_modules ) && 100 > $grade_data[0]['grade'] ) {
				...
				$stop_id    = get_post_ancestors( $module_id );
				$stop_id    = $stop_id[0];
				...
				Stop_Module::save_response_grade( $response_id, $grade, $student_id, $field_id, $stop_id, $module_id );
			}
		...
		public static function get_ungraded_response_count( $field_id = '' ) {
			...
			if ( $field_id == '' ) {
				...
				foreach ( $ungraded_responses as $key => $ungraded_response ) {
					...
					// Only show count for fields an Instructor can actually assess
					$field_id = get_post_meta( $ungraded_response->ID, 'field_id', true );
					if ( ! FieldPress_Capabilities::is_field_instructor( $field_id ) ) {
						continue;
					}
					...
					if ( $module ) {
						...
						$stop      = get_post( $module->post_parent );
						$field_id = $stop->post_parent;
						// Count only answers from students that are still enrolled in the field trip.
						if ( ! get_user_option( 'enrolled_field_date_' . $field_id, $ungraded_response->post_author ) ) {
							unset( $ungraded_responses[ $key ] );
							continue;
						}
					}
		...
		public static function grade_status_and_resubmit(
			...
			$preview_data = FieldPress::instance()->preview_data;
					...
					<div class="module_grade_left">
						...
						if ( $grade_value < $minimum_grade ) {
							if ( ( $number_of_answers < $limit_attempts_value ) || $limit_attempts_value == - 1 || $allow_free_resubmit ) {
								...
								$stop_id      = wp_get_post_parent_id( $data->ID );
								$field_id    = get_post_meta( $stop_id, 'field_id', true );
								$module_id    = $data->ID;
								$paged        = isset( $wp->query_vars['paged'] ) ? absint( $wp->query_vars['paged'] ) : 1;
								$permalink    = trailingslashit( trailingslashit( get_permalink( $stop_id ) ) . 'page/' . trailingslashit( $paged ) );
								if( empty( $_GET['resubmitting']) ){
									$resubmit_url = $permalink . '?resubmit_answer=' . $last_public_response->ID . '&resubmit_redirect_to=' . $permalink . '&m=' . $module_id . '&c=' . $field_id . '&u=' . $stop_id;
									?>
									...
									$resubmit_url = $permalink . '?resubmit_cancel=' . $previous_response . '&resubmit_redirect_to=' . $permalink . '&m=' . $module_id . '&c=' . $field_id . '&u=' . $stop_id;
		...
		public static function create_auto_draft( $stop_id ) {
			global $user_id;
		...
		function duplicate( $module_id = '', $stop_id = '' ) {
			global $wpdb;
			...
			$new_module    = Stop_Module::get_module( $module_id );
			...
			$new_module->post_parent = $stop_id;
