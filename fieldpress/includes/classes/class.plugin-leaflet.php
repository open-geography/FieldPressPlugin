 ...
 */
if ( ! class_exists( 'CP_Plugin_Activation_Leaflet' ) ) {

	/**
	 * Automatic plugin installation and activation library.
	 * The plugins can be either pre-packaged, downloaded from the WordPress
	 */
	class CP_Plugin_Activation_Leaflet {
		...
		public function __construct() {
			global $fieldpress, $plugin_dir, $screen_base;
			...
			$this->tab  = 'cp-leafletmapmaker';
			$this->menu = $screen_base . '_settings&tab=' . $this->tab;

			$this->plugins['leafletmapmaker'] = array(
					'name'           => 'LeafletMapMaker',
					// The plugin name.
					'slug'           => 'leaflet-maps-marker',
					// The plugin slug (typically the folder name).
					'base_path'      => 'leaflet-maps-marker/leaflet-maps-marker.php',
					//'source'         => FieldPress::instance()->plugin_dir . 'includes/plugins/' . FieldPress::instance()->leaflet_file,
					'source'         => is_object( $fieldpress ) ? $fieldpress->plugin_dir . 'includes/plugins/' . $fieldpress->leaflet_file : '',
					// The plugin source.
					'source_message' => __( 'Included in the FieldPress Plugin', 'cp' ),
...
//				);
			...
			$this->plugin = $this->plugins['leafletmapmaker'];
...
//			}
			...
			$this->strings = array(
				...
				'notice_can_install_recommended'  => sprintf( __( 'Install %1$s plugin in order to add maps to posts.', 'cp' ), $this->plugin['name'] ),
				'notice_can_activate_recommended' => '',
				'install_link'                    => sprintf( __( 'Install %1$s', 'cp' ), $this->plugin['name'] ),
				'activate_link'                   => sprintf( __( 'Activate %1$s plugin in order to add maps to posts', 'cp' ), $this->plugin['name'] ),
				'return'                          => __( 'Return to Leaflet Maps Maker Installer', 'cp' ),
				...
				$actions = array(
					'install' => sprintf(
						...
						esc_url_raw( add_query_arg(
							array(
								'page'              => CP_Plugin_Activation_Leaflet::$instance->menu,
		 ...
		 */
		public function install_plugins_page_leaflet() {
					...
					<table class="wp-list-table widefat fixed plugins">
						...
						<tr class="alternate">
							...
							<td class="status column-status">
								<?php echo $this->get_plugin_status( 'leaflet-maps-marker' ); ?>
							</td>
						</tr>
						</tbody>
					</table>
		...
		public static function get_instance() {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof CP_Plugin_Activation_Leaflet ) ) {
				self::$instance = new CP_Plugin_Activation_Leaflet();
			}
...
global $cp_plugin_activation_leaflet;
$cp_plugin_activation_leaflet = new CP_Plugin_Activation_Leaflet();