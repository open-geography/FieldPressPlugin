...
/**
 * Hooks Field actions and filters according to Field settings.
 */
class Field_Settings {

	function __construct() {
		add_filter( 'fieldpress_field_enrollment_types', array( &$this, 'enrollment_types' ) );
	...
	public function enrollment_types( $enrollment_types ) {
		...
		if ( cp_user_can_register() ) {
			...
			$enrollment_types['prerequisite'] = __( 'Anyone who completed the prerequisite field', 'cp' );
		} else {
			$enrollment_types['registered']   = __( 'Registered User', 'cp' );
			$enrollment_types['passcode']     = __( 'Registered user with a pass code', 'cp' );
			$enrollment_types['prerequisite'] = __( 'Registered user who completed the prerequisite field', 'cp' );
		}
