...
if ( ! class_exists( 'CP_Basic_Certificate' ) ) {
	...
	class CP_Basic_Certificate {
		...
		private static $default_css = ".basic_certificate{\n}\n\n.basic_certificate .first_name {\n}\n\n.basic_certificate .last_name {\n}\n\n.basic_certificate .field_name {\n}\n\n.basic_certificate .completion_date {\n}\n\n.basic_certificate .certificate_number {\n}\n\n.basic_certificate .stop_list {\n}\n\n";

		public static function init_settings() {
			add_filter( 'fieldpress_settings_new_menus', array( 'CP_Basic_Certificate', 'add_settings_item' ) );
			add_action( 'fieldpress_settings_menu_basic_certificate', array( 'CP_Basic_Certificate', 'render_settings') );
			//add_action( 'fieldpress_email_settings', array( 'CP_Basic_Certificate', 'render_email_settings') );
			//add_action( 'fieldpress_update_settings', array( 'CP_Basic_Certificate', 'process_email_settings'), 10, 2 );
		}

		public static function init_front() {
			add_action( 'fieldpress_pre_parse_action', array( 'CP_Basic_Certificate', 'redirect_to_pdf' ) );
		}

		public static function redirect_to_pdf() {
			if( isset( $_GET['action'] ) && 'view_certificate' == $_GET['action'] && isset( $_GET['field_id'] ) ) {
				$field_id = (int) $_GET['field_id'];
				if( $field_id > 0 ) {
					$certificate = CP_Basic_Certificate::make_pdf( get_current_user_id(), $field_id, true );
			...
			<form action='' method='post'>
				...
				<div class="postbox">
					...
					<div class="inside">
						<table class="form-table">
									...
									<a class="help-icon" href="javascript:;"></a>
										<div class="tooltip">
											...
											<div class="tooltip-content">
												<?php _e( 'Adds a "Certificate" link to completed field trips on the student dashboard.', 'cp' ); ?>
											</div>
										</div>
								</th>
								<td>
							...
							<!--<tr valign="top">-->
								<!--<th scope="row">--><?php //_e( 'Email certificate when field is completed.', 'cp' ); ?><!--</th>-->
								<!--<td>-->
				...
				<div class="postbox">
					...
					<div class="inside">
						...
						<p class="description"><?php _e( 'These codes will be replaced with actual data: FIRST_NAME, LAST_NAME, FIELD_NAME, COMPLETION_DATE, CERTIFICATE_NUMBER, STOP_LIST', 'cp' ); ?></p>
						<table class="form-table">
							<tbody id="items">
								<tr>
									<td>
								...
								<tr>
									<td>
										<p>
											...
											<span class="description"><?php esc_html_e( 'Can be any CSS stops. E.g. "0.2em"', 'cp'); ?></span>
										</p>
		...
		private static function render_css_setting() {
			...
			<div class="postbox">
				...
				<div class="inside">
					<p class="description"><?php _e( 'You can specify individual CSS rules for each of the certificate fields.<br />The follow CSS classes are available: .first_name, .last_name, .field_name, .completion_date, .certificate_number, .stop_list.<br />They are all wrapped in the .basic_certificate DIV tag.', 'cp' ); ?></p>
					<table class="form-table">
						<tbody id="items">
						<tr>
							<td>
		...
		?>
			<div class="postbox">
				...
				<div class="inside">
					<p class="description"><?php _e( 'E-mail to send certificate to student upon field completion. (if enabled)', 'cp' ); ?></p>
					<table class="form-table">
						<tbody id="items">
						<tr>
						...
						<tr>
							...
							<td>
								<p class="description"><?php _e( 'These codes will be replaced with actual data: FIRST_NAME, LAST_NAME, COMPLETION_DATE, CERTIFICATE_NUMBER, STOP_LIST, FIELD_NAME, FIELD_EXCERPT, FIELD_ADDRESS, WEBSITE_ADDRESS, WEBSITE_NAME', 'cp' ); ?></p>
								<?php
		...
		public static function make_pdf( $student, $field, $url = false ) {

			// Use FieldPress_PDF which extends TCPDF
			require_once( FieldPress::instance()->plugin_dir . 'includes/classes/class.fieldpress-pdf.php' );

			// Get the objects from IDs if passed as IDs.
			if( ! is_object( $field ) && 0 != (int) $field ) {
			...
			$field_completed_details = self::_get_fields( $student, $field );

			$certificate_title = sprintf( __('Certificate %s', 'cp' ), $field_completed_details['certificate_number'] );
			...
			$html = self::_replace_fields( $html, $student, $field, true, $field_completed_details );
			...
			return $pdf->make_pdf( $html, array(
					...
					'uid' => $field_completed_details['certificate_number'],
					'url' => $url,
				)
			);
		}
		...
		public static function option( $key, $value = null, $default = null ) {
			$options = get_option( 'fieldpress_basic_certificate' );
			if( empty( $options ) ) {
			...
			if( null === $value ) {
				...
				$options[ $key ] = $value;
				update_option( 'fieldpress_basic_certificate', $options );
		...
		private static function _default_certificate_content() {
			...
			$default_certification_content = sprintf( __( '%1$s %2$s
				has successfully completed the field trip
		...
		private static function _default_email_subject() {
			return sprintf( __( '[%s] Congratulations. You passed your field.', 'cp' ), get_option( 'blogname' ) );
		}
		...
		private static function _default_email_content() {
			...
			$default_instructor_invitation_email = sprintf( __(
				...
				Congratulations! You have completed the field trip: %2$s

				Please find attached your certificate of completion.'
				, 'cp' ), 'FIRST_NAME', 'FIELD_NAME'
			);

			return get_option( 'instructor_invitation_email', $default_instructor_invitation_email );
		}

		private static function _get_fields( $student, $field ) {

			// Note ID and id is inconsistent
			$field_completed = Student_Completion::is_field_complete( $student->ID, $field->id );
			$field_completed_details = get_user_option( '_field_' . $field->id . '_completed', $student->ID );

			if( empty( $field ) || empty( $student ) || ! $field_completed ) {
				return false;
			}
			...
			$show_stops = false;
			$stop_list = '<ul class="stop_list">';
			foreach( $stops as $stop ) {
				$show_stops = true;
				$stop_list .= '<li>' . sanitize_text_field( $stop->post_title ) . '</li>';
			}
			$stop_list .= '</ul>';
			if( ! $show_stops ) {
				$stop_list = '';
			...
			$fields = array(
				...
				'completion_date' => date_i18n( get_option( 'date_format '), $field_completed_details['date_completed'] ),
				'certificate_number' => $field_completed_details['certificate_number'],
				'stop_list' => $stop_list,
			);
		...
		private static function _replace_fields( $content, $student, $field, $html = true, $fields = false ) {

			if( ! $fields ) {
				$fields = self::_get_fields( $student, $field );
			}
		...
		public static function get_certificate_link( $student_id, $field_id, $link_title, $pre = '', $post = '', $show_link = false ) {

			if( ! $show_link ) {
				$show_link = CP_Basic_Certificate::option( 'basic_certificate_enabled' );
				$show_link = ! empty( $show_link ) ? true : false;
				$show_link = FieldPress_Capabilities::is_pro() ? $show_link : false;
			}

			if( $show_link ) {
				if ( Student_Completion::is_field_complete( $student_id, $field_id ) ) {
					$certificate_permalink = add_query_arg( array( 'action' => 'view_certificate', 'field_id' => $field_id ), get_permalink( $field_id ) );
