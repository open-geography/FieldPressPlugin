...
if ( ! class_exists( 'Field_Calendar' ) ) {
	class Field_Calendar {

		// Example:
		// $cal = new Field_Calendar( array( 'field_id'=>4 ) );
		...
		private $month = false;
		private $year = false;
		private $field_id = false;
		private $field_start = false;
		private $field_start_day = false;
		private $field_end = false;
		private $field_end_day = false;
		private $field_no_end = false;
		...
		function __construct( $args ) {
			...
			if ( $field_id && 'false' != $field_id ) {
				...
				$this->field_start_day = $start_date;
				$start_date             = getdate( strtotime( str_replace( '-', '/', $start_date ) ) );
				$this->field_start     = $start_date;
				$end_date               = get_post_meta( $field_id, 'field_end_date', true );
				$this->field_end_day   = $end_date;
				$end_date               = getdate( strtotime( str_replace( '-', '/', $end_date ) ) );
				$this->field_end       = $end_date;
				$this->field_no_end    = 'off' == get_post_meta( $field_id, 'open_ended_field', true ) ? false : true;
				...
				$this->previous_month = $this->get_previous_month( $date, $this->field_start );
				$this->next_month     = $this->get_next_month( $date, $this->field_end );
				// If today (or given date) is bigger than field end date, then use the field trip start date
				if ( ( strtotime( $date['year'] . '/' . $date['mon'] . '/' . $date['mday'] ) > strtotime( str_replace( '-', '/', $this->field_end_day ) ) ) &&
				     $this->field_start['mon'] != $date['mon'] && ! $this->field_no_end
				) {
			...
			$this->year           = $year;
			$this->month          = $month;
			$this->field_id      = $field_id ? $field_id : false;
			$this->date_indicator = sanitize_text_field( $date_indicator );
		}

		function create_calendar( $pre = '«', $next = '»' ) {
			global $wp_locale;
			$calendar = '<div class="field-calendar" data-fieldid="' . $this->field_id . '">';
			...
			$calendar .= "<table class='field-calendar-body " . $this->date_indicator . "'>";
			$calendar .= "<caption>";
			...
			while ( $current_day <= $this->number_of_days ) {
				...
				if ( $this->field_id ) {
					if ( strtotime( str_replace( '-', '/', $date ) ) == strtotime( str_replace( '-', '/', $this->field_start_day ) ) ) {
						$class = 'field-start-date';
					}
					if ( $this->field_no_end &&
					     ( strtotime( str_replace( '-', '/', $date ) ) > strtotime( str_replace( '-', '/', $this->field_start_day ) ) )
					) {
						$class = 'field-open-date';
					}
					if ( ( strtotime( str_replace( '-', '/', $date ) ) < strtotime( str_replace( '-', '/', $this->field_end_day ) ) ) &&
					     ( strtotime( str_replace( '-', '/', $date ) ) > strtotime( str_replace( '-', '/', $this->field_start_day ) ) )
					) {
						$class = 'field-active-date';
					}
					if ( strtotime( str_replace( '-', '/', $date ) ) == strtotime( str_replace( '-', '/', $this->field_end_day ) ) &&
					     ! $this->field_no_end
					) {
		...
		function get_next_month( $date, $end_date = false ) {
			if ( $date['mon'] < $end_date['mon'] || $date['year'] < $end_date['year'] || $this->field_no_end ) {
				$next_year  = $date['year'];
