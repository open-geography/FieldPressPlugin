...
if ( ! class_exists( 'FieldPress_Virtual_Page' ) ) {

	class FieldPress_Virtual_Page {
		...
		// const TITLE_FIELD_LIST = __( 'Field Trips', 'cp' );
		...
		function virtualPage( $posts ) {
			...
			$old_post_slug_id = get_posts( array( 'post_type'     => array( 'post', 'page', 'stop', 'field' ),
			                                      'name'          => $wp->request,
			                                      'post_per_page' => 1,
			                                      'fields'        => 'ids'
			) );
