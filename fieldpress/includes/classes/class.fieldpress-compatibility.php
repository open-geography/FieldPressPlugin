 ...
 *
 */
if ( ! class_exists( 'FieldPress_Compatibility' ) ) {

	/**
	 * FieldPress class for dealing with WordPress version compatibility
	 *
	 * @since 1.2.1
	 *
	 */
	class FieldPress_Compatibility {
		...
		function __construct() {
			$this->plugin_url = $GLOBALS['fieldpress_url'];
			$this->version    = $GLOBALS['fieldpress_version'];
			...
			if ( is_admin() ) {
				...
				add_action( 'fieldpress_editor_compatibility', array( &$this, 'fieldpress_editor_compatibility' ) );

				// Fix some anomalies on stop builder page
				add_action( 'admin_init', array( &$this, 'alter_stop_builder_editor' ) );
			}
			add_action( 'fieldpress_editor_compatibility', array( &$this, 'fieldpress_editor_compatibility' ) );
			// Public area
		...
		/**
		 * Alter editor for Stop pages.
		 *
		 * Because of the complexities of the stop builder we might need to suppress
		 * or alter some editor options.
		 *
		 * @since 1.2.1
		 */
		public function alter_stop_builder_editor() {
			if ( isset( $_GET['page'] ) && 'field_details' == $_GET['page'] && isset( $_GET['tab'] ) && 'stops' == $_GET['tab'] ) {
				/*
		 ...
		 * @since 1.2.1
		 */
		public function fieldpress_editor_compatibility() {
			...
			switch ( $this->min_version ) {
				...
				case 3.9:
					add_filter( 'fieldpress_element_editor_args', array(
						&$this,
						'cp_element_editor_args_39plus'
					), 10, 3 );
					add_action( 'fieldpress_editor_options', array( &$this, 'prepare_fieldpress_editor_39plus' ) );
					break;
				...
				case 3.8:
					...
					add_filter( 'fieldpress_format_tinymce_plugins', array(
						&$this,
						'cp_format_tinymce_plugins_38'
					), 10, 1 );
					add_action( 'fieldpress_editor_options', array( &$this, 'prepare_fieldpress_editor_38' ) );
					break;
			}
			 ...
			 *
			 * Keeps consistency across Field Trip Setup and stop setup.
			 *
			 * @since 1.0.0
			 */
		...
		function admin_header_actions() {
			...
			if ( $this->min_version >= 3.8 ) {
				wp_register_style( 'cp-38', FieldPress::instance()->plugin_url . 'css/admin-icon.css' );
			...
			if ( $page == 'fields' || $page == 'field_details' || $page == 'instructors' || $page == 'students' || $page == 'assessment' || $page == 'reports' || $page == 'settings' || ( isset( $_GET['taxonomy'] ) && $_GET['taxonomy'] == 'field_category' ) ) {
		...
		public function admin_footer_scripts( $hook ) {
			...
			if ( 'fieldpress-pro_page_field_details' == $hook ) {
				wp_enqueue_script( 'fieldpress_fix_editor', $this->plugin_url . 'js/footer-editor-fixes.js', array( 'jquery' ), $this->version, true );
			}
		}
		...
		// FieldPress CSS styles for TinyMCE
		function mce_editor_style( $url ) {
			...
			if ( in_array( $page, $detect_pages ) ) {
				...
				$url .= FieldPress::instance()->plugin_url . 'css/editor_style_fix.css,';

				if ( 3.9 <= (double) $wp_version ) {

				} else {
					$url .= FieldPress::instance()->plugin_url . 'css/editor_style_fix_38.css,';
				}
			}
		...
		/* Retrieve wp_editor dynamically ( using in stop admin ) */
		...
		function cp_format_TinyMCE( $in ) {
			$plugins = apply_filters( 'fieldpress_format_tinymce_plugins', $this->get_plugins() );
		...
		function prepare_fieldpress_editor_39plus() {
			//array( 'inlinepopups', 'tabfocus', 'paste', 'media', 'fullscreen', 'wordpress', 'wpeditimage', 'wpgallery', 'wplink', 'wpdialogs', 'textcolor', 'hr' )
			wp_localize_script( 'fields_bulk', 'fieldpress_editor', array(
				'plugins'   => apply_filters( 'fieldpress_format_tinymce_plugins', $this->get_plugins() ),
				'toolbar'   => $this->get_buttons(),
				'theme'     => apply_filters( 'fieldpress_editor_theme', 'modern' ), // filter it for themers
				'skin'      => apply_filters( 'fieldpress_editor_skin', 'wp_theme' ), // filter it for themers
		...
		function prepare_fieldpress_editor_38() {
			wp_localize_script( 'fields_bulk', 'fieldpress_editor', array(
				'plugins'   => apply_filters( 'fieldpress_format_tinymce_plugins', $this->get_plugins() ),
				'toolbar'   => $this->get_buttons(),
				'theme'     => apply_filters( 'fieldpress_editor_theme', 'advanced' ), // filter it for themers
				'skin'      => apply_filters( 'fieldpress_editor_skin', 'wp_theme' ), // filter it for themers
...
$fieldpress_compatibility = new FieldPress_Compatibility();