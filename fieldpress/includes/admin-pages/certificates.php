...
if ( isset( $_POST['add_new_template'] ) ) {
	if ( check_admin_referer( 'save_template' ) ) {
		if ( current_user_can( 'fieldpress_create_certificates_cap' ) || current_user_can( 'manage_options' ) ) {
...
if ( isset( $_GET['action'] ) && $_GET['action'] == 'edit' && ( current_user_can( 'fieldpress_update_certificates_cap' ) || current_user_can( 'manage_options' ) ) ) {
	$post_id               = (int) $_GET['ID'];
...
if ( isset( $_GET['action'] ) && $_GET['action'] == 'delete' ) {
	if ( ! isset( $_POST['_wpnonce'] ) ) {
		...
		if ( current_user_can( 'fieldpress_delete_certificates_cap' ) || current_user_can( 'manage_options' ) ) {
...
<?php if ( ! isset( $_GET['action'] ) || isset( $_POST['template_id'] ) ) { ?>
	...
	<table cellspacing="0" class="widefat shadow-table">
		...
		foreach ( $wp_templates_search->get_results() as $template ) {
			...
			$template_object = apply_filters( 'fieldpress_template_object_details', $template_obj->details );
			...
			<tr id='user-<?php echo $template_object->ID; ?>' <?php echo $style; ?>>
				...
				foreach ( $columns as $key => $col ) {
					...
					<?php
						...
						?>
						<td>
							<?php echo apply_filters( 'fieldpress_template_field_value', $template_object->$key ); ?>
						</td>
					<?php
					}
				}
			...
			<div id="post-body" class="metabox-holder columns-2">
				<div id="post-body-content">
					...
					<div id="wp-content-wrap" class="wp-core-ui wp-editor-wrap tmce-active has-dfw certificate-layout">
						...
						<div class="rows">
							<?php for ( $i = 1; $i <= apply_filters( 'fieldpress_certificate_template_row_number', 20 ); $i ++ ) { ?>
						...
						<input type="hidden" name="rows_number_post_meta" value="<?php echo apply_filters( 'fieldpress_certificate_template_row_number', 20 ); ?>"/>
					</div>
					<!--wp-content-wrap-->
				</div>
				<!--post-body-content-->
								...
								<div class="submitbox" id="submitpost">
									...
									<div id="minor-publishing">
										...
										<div id="minor-publishing-actions">
											...
											<?php
											do_action( 'fieldpress_template_document_settings' );
											?>
										</div>
										<div class="clear"></div>
									</div>
