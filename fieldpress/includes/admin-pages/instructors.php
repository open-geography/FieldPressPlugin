	...
	foreach ( $_POST['users'] as $user_value ) {
		...
		if ( is_numeric( $user_value ) ) {
			...
			switch ( addslashes( $action ) ) {
				...
				case 'unassign':
					$instructor->unassign_from_all_fields();
					$message = __( 'Selected instructors has been unassigned from all field trips successfully.', 'cp' );
					break;
			}
		}
	}
				...
				<div class="alignleft actions">
					<?php if ( current_user_can( 'manage_options' ) ) { ?>
						<select name="action">
							...
							<option value="unassign"><?php _e( 'Unassign from all field trips', 'cp' ); ?></option>
						</select>
		...
		<table cellspacing="0" class="widefat fixed shadow-table stop-control-buttons">
			...
			foreach ( $wp_user_search->get_results() as $user ) {
				...
				<tr id='user-<?php echo $user_object->ID; ?>' class="<?php echo $style; ?>">
					...
					<td class="column-fields <?php echo $style; ?>"><?php echo $user_object->fields_number; ?></td>
