			...
			switch ( addslashes( $action ) ) {
				case 'delete':
					if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_students_cap' ) ) {
						...
						$message = __( 'Selected students have been withdrawn from all field trips successfully. Note: The user accounts still exist.', 'cp' );
					}
					break;
				...
				case 'withdraw':
					if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_withdraw_students_cap' ) ) {
						...
						$message = __( 'Selected students have been withdrawn from all field trips successfully.', 'cp' );
					}
					break;
			}
		}
...
if ( isset( $_GET['action'] ) && $_GET['action'] == 'delete' && isset( $_GET['student_id'] ) && is_numeric( $_GET['student_id'] ) ) {
	...
	$student->delete_student();
	$message = __( 'Selected student has been withdrawn from all field trips successfully. Note: The user account still exists.', 'cp' );
}
...
// Query the users
	...
	<div class="wrap nosubsub students cp-wrap">
		...
		<h2><?php _e( 'Students', 'cp' ); ?><?php if ( current_user_can( 'manage_options' ) ) { ?>
				<a class="add-new-h2" href="user-new.php"><?php _e( 'Add New', 'cp' ); ?></a><?php } ?><?php if ( current_user_can( 'fieldpress_add_new_students_cap' ) && ! current_user_can( 'manage_options' ) ) { ?>
		...
		<div class="tablenav tablenav-top">
			...
			<form method="post" action="<?php echo esc_attr( admin_url( 'admin.php?page=' . $page ) ); ?>" id="posts-filter">
				...
				<div class="alignleft actions">
					<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_withdraw_students_cap' ) || current_user_can( 'fieldpress_delete_students_cap' ) ) { ?>
						<select name="action">
							<option selected="selected" value=""><?php _e( 'Bulk Actions', 'cp' ); ?></option>
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_students_cap' ) ) { ?>
								<option value="delete"><?php _e( 'Delete', 'cp' ); ?></option>
							<?php } ?>
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_withdraw_students_cap' ) ) { ?>
								<option value="withdraw"><?php _e( 'Withdraw from all field trips', 'cp' ); ?></option>
							<?php } ?>
						</select>
		...
		?>
		<table cellspacing="0" class="widefat fixed shadow-table stop-control-buttons">
			<thead>
			<tr>
				...
				<tr id='user-<?php echo $user_object->ID; ?>' class="<?php echo $style; ?>">
					...
					<td class="column-user-fullname visible-small visible-extra-small <?php echo $style; ?>">
						...
						<div class="visible-extra-small">
							...
							<span class="latest_activity"><?php echo( isset( $user_object->latest_activity ) && $user_object->latest_activity !== '' ? date_i18n( 'Y-m-d h:i:s', $user_object->latest_activity ) : __( 'N/A', 'cp' ) ); ?></span> <?php if ( $fieldpress->user_is_currently_active( $user_object->ID ) ) { ?>
					...
					<td class="column-fields <?php echo $style; ?>" style="padding-left: 30px;"><?php echo $user_object->fields_number; ?></td>
