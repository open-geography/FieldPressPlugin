		...
		foreach ( $_POST['notifications'] as $notification_value ) {
			if ( is_numeric( $notification_value ) ) {
				...
				switch ( addslashes( $action ) ) {
					case 'publish':
						if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_change_notification_status_cap' ) || ( current_user_can( 'fieldpress_change_my_field_notification_cap' ) && $notification_object->post_author == get_current_user_id() ) ) {
					...
					case 'unpublish':
						if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_change_notification_status_cap' ) || ( current_user_can( 'fieldpress_change_my_notification_status_cap' ) && $notification_object->post_author == get_current_user_id() ) ) {
					...
					case 'delete':
						if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_notification_cap' ) || ( current_user_can( 'fieldpress_delete_my_notification_cap' ) && $notification_object->post_author == get_current_user_id() ) ) {
...
// Query the notifications
	...
	<div class="wrap nosubsub notifications cp-wrap">
		...
		<h2><?php _e( 'Notifications', 'cp' ); ?><?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_create_notification_cap' ) || current_user_can( 'fieldpress_create_my_notification_cap' ) || current_user_can( 'fieldpress_create_my_assigned_notification_cap' ) ) { ?>
		...
		<div class="tablenav tablenav-top">
			...
			<form method="post" action="<?php echo esc_attr( admin_url( 'admin.php?page=' . $page ) ); ?>" id="posts-filter">
				...
				<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_change_notification_status_cap' ) || current_user_can( 'fieldpress_delete_notification_cap' ) ) { ?>
					<div class="alignleft actions">
						<select name="action">
							<option selected="selected" value=""><?php _e( 'Bulk Actions', 'cp' ); ?></option>
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_change_notification_status_cap' ) ) { ?>
								<option value="publish"><?php _e( 'Publish', 'cp' ); ?></option>
								<option value="unpublish"><?php _e( 'Private', 'cp' ); ?></option>
							<?php } ?>
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_notification_cap' ) ) { ?>
		...
		if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_notification_cap' ) || ( current_user_can( 'fieldpress_delete_my_notification_cap' ) ) ) {
		...
		<table cellspacing="0" class="widefat shadow-table">
			...
			foreach ( $wp_notification_search->get_results() as $notification ) {
				...
				?>
				<?php
				if ( isset( $notification_object->field_id ) && $notification_object->field_id !== '' ) {
					$field      = new Field( $notification_object->field_id );
					$field_name = $field->details->post_title;
				} else {
					$field_name = __( 'All Field Trips', 'cp' );
				}
				?>
				<tr id='user-<?php echo $notification_object->ID; ?>' class="<?php echo $style; ?>">
					...
					<td class="column-notification-title <?php echo $style; ?>">
						...
						<div class="visible-small visible-extra-small">
							<strong><?php _e( 'Field Trip:', 'cp' ); ?></strong> <?php echo $field_name; ?></div>
						<div class="visible-small visible-extra-small">
						...
						<div class="field_excerpt"><?php echo cp_get_the_field_excerpt( $notification_object->ID ); ?></div>
						<div class="row-actions">
							...
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_change_notification_status_cap' ) || ( current_user_can( 'fieldpress_change_my_notification_status_cap' ) && $notification_object->post_author == get_current_user_id() ) ) { ?>
							...
							<?php } ?>
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_notification_cap' ) || ( current_user_can( 'fieldpress_delete_my_notification_cap' ) && $notification_object->post_author == get_current_user_id() ) ) { ?>
								<span class="field_remove"><a href="<?php echo wp_nonce_url( admin_url( 'admin.php?page=notifications&action=delete&notification_id=' . $notification_object->ID ), 'delete_notification_' . $notification_object->ID, 'cp_nonce' ); ?>" onClick="return removeNotification();"><?php _e( 'Delete', 'cp' ); ?></a> | </span>
							<?php } ?>
						</div>
					</td>
					<td class="column-field <?php echo $style; ?>"> <?php echo $field_name; ?> </td>
					...
					<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_notification_cap' ) || ( current_user_can( 'fieldpress_delete_my_notification_cap' ) ) ) { ?>
			...
			if ( count( $wp_notification_search->get_results() ) == 0 ) {
				...
				<tr>
					<td colspan="6">
						<div class="zero-fields"><?php _e( 'No notifications found.', 'cp' ) ?></div>
					</td>
				</tr>
			<?php
			}
