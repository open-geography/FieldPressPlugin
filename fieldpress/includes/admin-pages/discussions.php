		...
		foreach ( $_POST['discussions'] as $discussion_value ) {
			if ( is_numeric( $discussion_value ) ) {
				...
				switch ( addslashes( $action ) ) {
					case 'delete':
						if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_discussion_cap' ) || ( current_user_can( 'fieldpress_delete_my_field_discussion_cap' ) && $discussion_object->post_author == get_current_user_id() ) ) {
...
// Query the discussions
	...
	<div class="wrap nosubsub cp-wrap">
		...
		<h2><?php _e( 'Discussions', 'cp' ); ?><?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_create_discussion_cap' ) || current_user_can( 'fieldpress_create_my_discussion_cap' ) || current_user_can( 'fieldpress_create_my_assigned_discussion_cap' ) ) { ?>
		...
		<div class="tablenav">
			...
			<form method="post" action="<?php echo esc_attr( admin_url( 'admin.php?page=' . $page ) ); ?>" id="posts-filter">
				...
				<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_discussion_cap' ) ) { ?>
		...
		if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_discussion_cap' ) || ( current_user_can( 'fieldpress_delete_my_field_discussion_cap' ) ) ) {
		...
		<table cellspacing="0" class="widefat shadow-table">
			...
			foreach ( $wp_discussion_search->get_results() as $discussion ) {
				...
				<tr id='user-<?php echo $discussion_object->ID; ?>' <?php echo $style; ?>>
					...
					<td <?php echo $style; ?>>
						...
						<div class="field_excerpt"><?php echo cp_get_the_field_excerpt( $discussion_object->ID ); ?></div>
						<div class="row-actions">
							...
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_discussion_cap' ) || ( current_user_can( 'fieldpress_delete_my_field_discussion_cap' ) && $discussion_object->post_author == get_current_user_id() ) ) { ?>
								<span class="field_remove"><a href="<?php echo wp_nonce_url( admin_url( 'admin.php?page=discussions&action=delete&discussion_id=' . $discussion_object->ID ), 'delete_discussion_' . $discussion_object->ID, 'cp_nonce' ); ?>" onClick="return removeDiscussion();"><?php _e( 'Delete', 'cp' ); ?></a></span>
							<?php } ?>
						</div>
					</td>
					<?php
					if ( isset( $discussion_object->field_id ) && $discussion_object->field_id !== '' ) {
						$field      = new Field( $discussion_object->field_id );
						$field_name = $field->details->post_title;
					} else {
						$field_name = __( 'All Field Trips', 'cp' );
					}
					?>
					<td <?php echo $style; ?>> <?php echo $field_name; ?> </td>

					<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_discussion_cap' ) || ( current_user_can( 'fieldpress_delete_my_field_discussion_cap' ) ) ) { ?>
						<td <?php echo $style; ?>>
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_delete_discussion_cap' ) || ( current_user_can( 'fieldpress_delete_my_field_discussion_cap' ) && $discussion_object->post_author == get_current_user_id() ) ) { ?>
			...
			if ( count( $wp_discussion_search->get_results() ) == 0 ) {
				...
				<tr>
					<td colspan="6">
						<div class="zero-fields"><?php _e( 'No discussions found.', 'cp' ) ?></div>
					</td>
				</tr>
			<?php
			}
