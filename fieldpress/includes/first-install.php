...
Sed egestas erat nec purus sollicitudin, vel elementum dolor blandit. Praesent interdum nulla quis augue accumsan, vel pulvinar ligula cursus. Curabitur id nisi nisl. Proin aliquam lorem id urna molestie, non eleifend diam interdum. Praesent ultricies metus lorem, quis tristique odio congue in. Phasellus imperdiet malesuada sem vel elementum. Pellentesque eu arcu in metus molestie cursus non ac lectus. Curabitur ut luctus orci, a rhoncus nisi. In luctus laoreet dui, vel ornare mauris ullamcorper vitae. Morbi dapibus faucibus orci, quis tempor lacus facilisis ac. Nam dignissim, leo at ultrices dignissim, ipsum dui consequat felis, a tincidunt odio odio ac libero.';
    $field_status = 'private';
    $field_hero_video_url = 'https://www.youtube.com/watch?v=y_bIr1yAELw&list=UULgqhMisF-ykzHZzuMEfV4Q';
    $field_class_size = 0; // zero means unlimited
    $field_who_can_enroll = 'anyone';
    $field_language = 'English';
    $field_open_ended = 'on';
    $field_open_ended_enrollment = 'on';
    $field_allow_discussion = 'on';
    $field_show_grade_page = 'off';

    //If there isn't any field, create one

    $_fields_count = ( wp_count_posts( 'field' )->publish ) + ( wp_count_posts( 'field' )->private );

    if ( $_fields_count == 0 ) {
        $new_field = array(
            'post_author' => $field_author,
            'post_excerpt' => cp_filter_content($field_excerpt),
            'post_content' => cp_filter_content($field_content),
            'post_status' => $field_status,
            'post_title' => cp_filter_content($field_title, true),
            'post_type' => 'field',
        );
        ...
        if ( $field_id != 0 ) {
            ...
            //update_user_option( $field_author, 'field_' . $field_id, $field_id, $global_option ); //only could be a user with "Intructor" role so admin can't be assigned

            //Set hero video
            update_post_meta( $field_id, 'field_video_url', cp_filter_content($field_hero_video_url, true) );

            //Set field field trip group size
            update_post_meta( $field_id, 'class_size', cp_filter_content($field_class_size) );

            //Set Who can Register to the field trip
            update_post_meta( $field_id, 'enroll_type', cp_filter_content($field_who_can_enroll, true) );

            //Set Field Trip Language
            update_post_meta( $field_id, 'field_language', cp_filter_content($field_language, true) );

            //Field is open-ended? Start date, but no end date.
            update_post_meta( $field_id, 'open_ended_field', cp_filter_content($field_open_ended) );

			//Students can enroll anytime?
			update_post_meta( $field_id, 'open_ended_enrollment', cp_filter_content($field_open_ended_enrollment) );

            //Allow Field Trip Discussion?
            update_post_meta( $field_id, 'allow_field_discussion', cp_filter_content($field_allow_discussion) );

            //Show grade page?
            update_post_meta( $field_id, 'allow_field_grades_page', cp_filter_content($field_show_grade_page) );
            ...
            $stop_title = 'Sample First Stop';
            $stop_content = 'Sample First Stop.';
            $stop_status = 'publish';

            $new_stop = array(
                'post_author' => $field_author,
                'post_content' => cp_filter_content($stop_content),
                'post_status' => $stop_status, //$post_status
                'post_title' => cp_filter_content($stop_title, true),
                'post_type' => 'stop',
            );

            $stop_id = wp_insert_post( $new_stop );
            ...
            if ( $stop_id != 0 ) {
                ...
                //Set parent field
                update_post_meta( $stop_id, 'field_id', $field_id );

                //Set stop order to it's ID for start
                update_post_meta( $stop_id, 'stop_order', $stop_id );

                //To force or not to force the stop completion in order to access to the next one
                update_post_meta( $stop_id, 'force_current_stop_completion', 'off' );

                //Date from when the stop is available / set current date for the example
                update_post_meta( $stop_id, 'stop_availability', date( 'Y-m-d', current_time( 'timestamp', 0 ) ) );
            }
            ...
            $stop_title = 'Pellentesque eu arcu in metus';
            $stop_content = 'Vestibulum tincidunt laoreet ultricies. Quisque quis diam orci. Pellentesque hendrerit, lectus sit amet pulvinar tristique, lorem lorem suscipit sapien, at sollicitudin augue nulla quis risus. Vestibulum vulputate sagittis risus id gravida.';
            $stop_status = 'publish';
            ...
            /*
             *
             * Insert elements for the first stop
             *
             */


            $stop_element = new Stop_Module();

            //Add an audio element

            $data->stop_id = $first_stop_id;
            ...
            $stop_element->update_module( $data );
            ...
            /*
             *
             * Insert elements for the second stop
             *
             */

            //Add a video element

            $data->stop_id = $second_stop_id;
            $data->title = 'Video Element Title';
            $data->content = 'Vivamus vulputate, ligula a tempus tempor, orci nulla interdum tortor, et malesuada augue nisi eget lacus.';
...
Donec sit amet erat eros. Mauris volutpat massa libero, pretium fermentum magna tempor quis. Nulla vel felis ligula. Sed dui velit, tristique at justo id, convallis rhoncus nisl. Aenean ut metus dictum, iaculis lacus nec, tincidunt ligula. Vivamus dictum odio sagittis sem consectetur porta. Nunc semper arcu ac arcu cursus, eget porta velit adipiscing. Suspendisse mattis neque in ipsum adipiscing, in convallis nulla laoreet. Quisque ac malesuada nunc, molestie dignissim sem.';
            ...
            /*
             *
             * Insert elements for the third stop
             *
             */

            //Add a text element

            $data->stop_id = $third_stop_id;
