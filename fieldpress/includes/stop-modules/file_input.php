...
class file_input_module extends Stop_Module {
	...
	public static function get_response_form( $user_ID, $response_request_ID, $show_label = true ) {
		...
		if ( count( (array) $response >= 1 ) ) {
			require_once( $fieldpress->plugin_dir . 'includes/classes/class.encryption.php' );
	...
	public static function front_main( $data ) {
		...
		if ( count( $response ) == 0 ) {
			global $fieldpress;
			if ( $fieldpress->is_preview( Stop_Module::get_module_stop_id( $data->ID ) ) ) {
				$enabled = 'disabled';
			} else {
				$enabled = 'enabled';
			}
		...
		<?php
		/* $stop_module_main = new Stop_Module();
	...
	function admin_main( $data ) {
		...
		?>
			...
			<div class="module-content">
				...
				<input type="hidden" name="<?php echo $this->name; ?>_id[]" class="stop_element_id" value="<?php echo esc_attr( isset( $data->ID ) ? $data->ID : '' ); ?>" />
	...
	function on_create() {
		$this->order       = apply_filters( 'fieldpress_' . $this->name . '_order', $this->order );
		$this->description = __( 'Add file upload blocks to the stop. Useful if students need to send you various files like essay, homework etc.', 'cp' );
	...
	function save_module_data() {
		global $wpdb, $last_inserted_stop_id, $save_elements;
				...
				if ( $module_value == $this->name ) {
					...
					if ( isset( $_POST[ $this->name . '_id' ] ) ) {
						foreach ( $_POST[ $this->name . '_id' ] as $key => $value ) {
							...
							$data->stop_id                         = ( ( isset( $_POST['stop_id'] ) and ( isset( $_POST['stop'] ) && $_POST['stop'] != '' ) ) ? $_POST['stop_id'] : $last_inserted_stop_id );
					...
					if ( $movefile ) {
						...
						if ( ! isset( $movefile['error'] ) ) {
							...
							if ( ! empty( $mandatory_answer ) && 'yes' == $mandatory_answer ) {
								Student_Completion::record_mandatory_answer( get_current_user_id(), $field_id, $stop_id, $response_id );
							}
						} else {
							?>
