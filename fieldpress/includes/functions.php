...
function cp_redirect_woo_to_field() {
	$redirect_woo_to_field = get_option( 'redirect_woo_to_field', 0 );
	if ( $redirect_woo_to_field == 0 ) {
		return false;
	} else {
		return true;
	}
...
function cp_redirect_mp_to_field() {
	$redirect_mp_to_field = get_option( 'redirect_mp_to_field', 0 );
	if ( $redirect_mp_to_field == 0 ) {
		return false;
	} else {
		return true;
	}
...
function cp_stop_uses_new_pagination( $stop_id = false ) {
	$stop_pagination_meta	 = get_post_meta( $stop_id, 'stop_pagination', true );
	$stop_pagination		 = isset( $stop_pagination_meta ) && !empty( $stop_pagination_meta ) && $stop_pagination_meta !== false ? true : false;

	return $stop_pagination;
}

function cp_get_id_by_post_name( $post_name, $post_parent = 0, $type = 'stop' ) {
	global $wpdb;
...
function cp_can_see_stop_draft() {
	if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_create_field_stop_cap' ) ) {
		return true;
	} else {
		return false;
	}
...
function cp_allowed_post_tags() {
	...
	return apply_filters( 'fieldpress_allowed_post_tags', $allowed_tags );
}
 ...
 */
function cp_set_last_visited_stop_page( $stop_id = false, $page_num = false, $student_id = false ) {
	...
	update_user_option( $student_id, 'last_visited_stop_' . $stop_id . '_page', $page_num, $global_option );
}
 ...
 * @todo: needs to be replaced and removed soon
 */
function cp_set_visited_field( $stop_id, $student_id = false ) {

	if ( !$student_id ) {
		$student_id = get_current_user_ID();
	}

	$field_id		 = wp_get_post_parent_id( $stop_id );
	$visited_fields = get_user_option( 'visited_field_stops_' . $field_id, $student_id );

	if ( $visited_fields === false ) {
		$visited_fields = $field_id;
	} else {
		$visited_fields = explode( ',', $visited_fields );
		if ( !in_array( $field_id, $visited_fields ) ) {
			$visited_fields[] = $field_id;
		}
		$visited_fields = implode( ',', $visited_fields );
	}
	$global_option = !is_multisite();
	update_user_option( $student_id, 'visited_field_stops_' . $field_id, $visited_fields, $global_option );
}

function cp_is_field_visited( $field_id, $student_id = false ) {
	if ( !$student_id ) {
		$student_id = get_current_user_ID();
	}
 ...
 * @todo: needs to be replaced and removed soon
 */
function cp_set_visited_stop_page( $stop_id = false, $page_num = false, $student_id = false, $field_id = false ) {
	...
	Student_Completion::record_visited_page( $student_id, $field_id, $stop_id, $page_num );
	...
	$global_option = !is_multisite();
	update_user_option( $student_id, 'visited_stop_pages_' . $stop_id . '_page', $visited_pages, $global_option );
	cp_set_visited_field( $stop_id, $student_id );
	cp_set_last_visited_stop_page( $stop_id, $page_num, $student_id );
}

function cp_get_number_of_stop_pages_visited( $stop_id = false, $student_id = false ) {
	if ( !$student_id ) {
		$student_id = get_current_user_ID();
	}
	$visited_pages = get_user_option( 'visited_stop_pages_' . $stop_id . '_page', $student_id );
...
function cp_get_last_visited_stop_page( $stop_id, $student_id = false ) {
	if ( !$student_id ) {
		$student_id = get_current_user_ID();
	}
	$last_visited_stop_page = get_user_option( 'last_visited_stop_' . $stop_id . '_page', $student_id );
	if ( $last_visited_stop_page ) {
		return $last_visited_stop_page;
	} else {
		return 1;
	}
}

function cp_get_order_field_id( $order_id ) {
	global $mp;
...
/**
 * Stop stop module pagination
 */
function fieldpress_stop_module_pagination( $stop_id, $pages_num, $check_is_last_page = false ) {
	global $wp, $wp_query, $paged, $fieldpress_modules, $fieldpress;

	if ( !isset( $stop_id ) ) {// || !is_singular()
		//<br clear="all">
...
//<br clear="all">
	...
	for ( $link_num = 1; $link_num <= $max; $link_num ++ ) {
		$enabled = "";
		if ( $fieldpress->is_preview( $stop_id, $link_num ) ) {
...
function fieldpress_stop_module_pagination_ellipsis( $stop_id, $pages_num ) {
	global $wp, $wp_query, $paged, $fieldpress_modules;
...
function fieldpress_stop_pages( $stop_id, $stop_pagination = false ) {
	...
	if ( $stop_pagination ) {
		...
		if ( $module_id > 0 ) {
			$pages_num = count( get_post_meta( $stop_id, 'page_title', true ) );
		...
		foreach ( $modules as $mod ) {
			if ( Stop_Module::get_module_type( $mod->ID ) == 'page_break_module' ) {
				$pages_num ++;
			}
		}
	}
...
function fieldpress_send_email( $email_args = array() ) {

	if ( $email_args[ 'email_type' ] == 'student_registration' ) {
		global $field_slug;
		$email_address	 = $email_args[ 'student_email' ];
		$subject		 = fieldpress_get_registration_email_subject();
		$fields_address = trailingslashit( home_url() ) . trailingslashit( $field_slug );
		...
		$message = fieldpress_get_registration_content_email();
		...
		if ( !function_exists( 'my_registration_from_function' ) ) {
			...
			function my_registration_from_function( $email ) {
				return fieldpress_get_registration_from_email();
			}
		...
		if ( !function_exists( 'my_registration_from_name_function' ) ) {
			...
			function my_registration_from_name_function( $name ) {
				return fieldpress_get_registration_from_name();
			}
	...
	if ( $email_args[ 'email_type' ] == 'enrollment_confirmation' ) {
		...
		$subject			 = fieldpress_get_enrollment_email_subject();
		$fields_address	 = trailingslashit( home_url() ) . trailingslashit( $field_slug );
		$field				 = new Field( $email_args[ 'field_id' ] );
		...
		$tags_replaces	 = array(
			...
			home_url(),
			$field->get_permalink(),
			$field->details->post_title,
			$email_args[ 'dashboard_address' ]
		);

		$message = fieldpress_get_enrollment_content_email();
		...
		if ( !function_exists( 'my_enrollment_from_function' ) ) {
			...
			function my_enrollment_from_function( $email ) {
				return fieldpress_get_enrollment_from_email();
			}
		...
		if ( !function_exists( 'my_enrollment_from_name_function' ) ) {
			...
			function my_enrollment_from_name_function( $name ) {
				return fieldpress_get_enrollment_from_name();
			}
	...
	if ( $email_args[ 'email_type' ] == 'student_invitation' ) {
		...
		$tags_replaces	 = array(
			...
			$email_args[ 'student_last_name' ],
			$field->details->post_title,
			$field->details->post_excerpt,
			$field->get_permalink(),
			home_url(),
			$field->details->passcode
		);

		if ( $email_args[ 'enroll_type' ] == 'passcode' ) {
			$message = fieldpress_get_invitation_content_passcode_email();
			$subject = fieldpress_get_invitation_passcode_email_subject();
		} else {
			$message = fieldpress_get_invitation_content_email();
			$subject = fieldpress_get_invitation_email_subject();
		}
		...
		if ( !function_exists( 'my_passcode_from_function' ) ) {
			...
			function my_passcode_from_function( $email ) {
				return fieldpress_get_invitation_passcode_from_email();
			}
		...
		if ( !function_exists( 'my_passcode_from_name_function' ) ) {
			...
			function my_passcode_from_name_function( $name ) {
				return fieldpress_get_invitation_passcode_from_name();
			}
	...
	if ( 'instructor_invitation' == $email_args[ 'email_type' ] ) {
		...
		if ( isset( $email_args[ 'field_id' ] ) ) {
			...
			$field_name	 = $field->details->post_title;
			$field_summary	 = $field->details->post_excerpt;

			// For unpublished fields.
			$permalink = '';
			if ( in_array( $field->details->post_status, array( 'draft', 'pending', 'auto-draft' ) ) ) {
				$permalink = FieldPress::instance()->get_field_slug( true ) . '/' . $field->details->post_name . '/';
			} else {
				$permalink = get_permalink( $email_args[ 'field_id' ] );
			}

			$field_address = $permalink;
		}

		$confirm_link = $field_address . '?action=field_invite&field_id=' . $email_args[ 'field_id' ] . '&c=' . $email_args[ 'invite_code' ] . '&h=' . $email_args[ 'invite_hash' ];
		...
		if ( !function_exists( 'my_instructor_invitation_from_function' ) ) {
			...
			function my_instructor_invitation_from_function( $email ) {
				return fieldpress_get_instructor_invitation_from_email();
			}
		...
		if ( !function_exists( 'my_instructor_invitation_from_name_function' ) ) {
			...
			function my_instructor_invitation_from_name_function( $name ) {
				return fieldpress_get_instructor_invitation_from_name();
			}
...
/* Get Student Invitation with Passcode to a Field E-mail data */

function fieldpress_get_invitation_passcode_from_name() {
	return get_option( 'invitation_passcode_from_name', get_option( 'blogname' ) );
}

function fieldpress_get_invitation_passcode_from_email() {
	return get_option( 'invitation_passcode_from_email', get_option( 'admin_email' ) );
}

function fieldpress_get_invitation_passcode_email_subject() {
	return get_option( 'invitation_passcode_email_subject', __( 'Invitation to a Field Trip ( Psss...for selected ones only )', 'cp' ) );
}

function fieldpress_get_invitation_content_passcode_email() {
	$default_invitation_content_passcode_email = sprintf( __( 'Hi %1$s,

we would like to invite you to participate in the field trip: "%2$s"

Since the field trip is only for selected ones, it is passcode protected. Here is the passcode for you: %6$s

What is all about:
%3$s

Check this page for more info on the field trip: %4$s

If you have any question feel free to contact us.

Yours sincerely,
%5$s Team', 'cp' ), 'STUDENT_FIRST_NAME', 'FIELD_NAME', 'FIELD_EXCERPT', '<a href="FIELD_ADDRESS">FIELD_ADDRESS</a>', '<a href="WEBSITE_ADDRESS">WEBSITE_ADDRESS</a>', 'PASSCODE' );

	return get_option( 'invitation_content_passcode_email', $default_invitation_content_passcode_email );
}

/* Get Student Invitation to a Field Trip E-mail data */

function fieldpress_get_invitation_from_name() {
	return get_option( 'invitation_from_name', get_option( 'blogname' ) );
}

function fieldpress_get_invitation_from_email() {
	return get_option( 'invitation_from_email', get_option( 'admin_email' ) );
}

function fieldpress_get_invitation_email_subject() {
	return get_option( 'invitation_email_subject', __( 'Invitation to a Field Trip', 'cp' ) );
}

function fieldpress_get_invitation_content_email() {
...
Yours sincerely,
%5$s Team', 'cp' ), 'STUDENT_FIRST_NAME', 'FIELD_NAME', 'FIELD_EXCERPT', '<a href="FIELD_ADDRESS">FIELD_ADDRESS</a>', '<a href="WEBSITE_ADDRESS">WEBSITE_ADDRESS</a>' );
...
Get started by exploring our fields here: %4$s

Yours sincerely,
%5$s Team', 'cp' ), 'STUDENT_FIRST_NAME', 'BLOG_NAME', '<a href="LOGIN_ADDRESS">LOGIN_ADDRESS</a>', '<a href="FIELDS_ADDRESS">FIELDS_ADDRESS</a>', '<a href="WEBSITE_ADDRESS">WEBSITE_ADDRESS</a>' );
...
Yours sincerely,
%5$s Team', 'cp' ), 'CUSTOMER_NAME', '<a href="FIELD_ADDRESS">FIELD_TITLE</a>', '<a href="STUDENT_DASHBOARD">' . __( 'Dashboard', 'cp' ) . '</a>', '<a href="FIELDS_ADDRESS">FIELD_TRIPS_ADDRESS</a>', 'BLOG_NAME' );
...
function fieldpress_get_enrollment_from_name() {
	return get_option( 'enrollment_from_name', get_option( 'blogname' ) );
}

function fieldpress_get_enrollment_from_email() {
	return get_option( 'enrollment_from_email', get_option( 'admin_email' ) );
}

function fieldpress_get_enrollment_email_subject() {
	return get_option( 'enrollment_email_subject', __( 'Enrollment Confirmation', 'cp' ) );
}

function fieldpress_get_enrollment_content_email() {
	$default_enrollment_content_email = sprintf( __( 'Hi %1$s,

Congratulations! You have enrolled in field trip "%2$s" successfully!

You may check all field trips you are enrolled in here: %3$s.

Or you can explore other field trips in your %4$s

Yours sincerely,
%5$s Team', 'cp' ), 'STUDENT_FIRST_NAME', '<a href="FIELD_ADDRESS">FIELD_TITLE</a>', '<a href="STUDENT_DASHBOARD">' . __( 'Dashboard', 'cp' ) . '</a>', '<a href="FIELDS_ADDRESS">FIELD_TRIPS_ADDRESS</a>', 'BLOG_NAME' );
...
function fieldpress_get_instructor_invitation_from_name() {
	return get_option( 'instructor_invitation_from_name', get_option( 'blogname' ) );
}

function fieldpress_get_instructor_invitation_from_email() {
...
Congratulations! You have been invited to become an instructor for the field trip: %2$s
...
%4$s
	'
	, 'cp' ), 'INSTRUCTOR_FIRST_NAME', 'FIELD_NAME', '<a href="CONFIRMATION_LINK">CONFIRMATION_LINK</a>', '<a href="WEBSITE_ADDRESS">WEBSITE_ADDRESS</a>'
	);
...
function cp_instructors_avatars( $field_id, $remove_buttons = true, $just_count = false ) {
	global $post_id, $wpdb;

	$content = '';

	//fieldpress_fields_cap
...
function cp_instructors_pending( $field_id, $has_capability ) {
	$content			 = '';
	...
	foreach ( $instructor_invites as $instructor ) {
		...
		$remove_button = $has_capability ? '<div class="instructor-remove"><a href="javascript:removePendingInstructor(\'' . $instructor[ 'code' ] . '\', ' . $field_id . ' );"><i class="fa fa-times-circle cp-move-icon remove-btn"></i></a></div>' : '';
...
function cp_instructors_drop_down( $class = '' ) {
	$content = '';
	$content .= '<select name="instructors" id="instructors" data-placeholder="' . __( 'Choose a Field Trip Instructor...', 'cp' ) . '" class="' . $class . '">';
...
function cp_cp_get_the_field_excerpt( $id = false, $length = 55 ) {
	global $post;
...
function cp_get_the_field_excerpt( $id = false, $length = 55 ) {
	global $post;
...
if ( !function_exists( 'cp_register_module' ) ) {
	...
	function cp_register_module( $module_name, $class_name, $section ) {
		global $fieldpress_modules, $fieldpress_modules_labels, $fieldpress_modules_descriptions, $fieldpress_modules_ordered;

		if ( !is_array( $fieldpress_modules ) ) {
			$fieldpress_modules = array();
		}

		if ( class_exists( $class_name ) ) {
			$class														 = new $class_name();
			$fieldpress_modules_labels[ $module_name ]					 = $class->label;
			$fieldpress_modules_descriptions[ $module_name ]			 = $class->description;
			$fieldpress_modules[ $section ][ $module_name ]			 = $class_name;
			$fieldpress_modules_ordered[ $section ][ $class->order ]	 = $class_name;
			ksort( $fieldpress_modules_ordered[ $section ] );
		} else {
			return false;
		}
	}
...
if ( !function_exists( 'cp_register_front_page_module' ) ) {
	...
	function cp_register_front_page_module( $module_name, $class_name, $section ) {
		global $fieldpress_front_page_modules, $fieldpress_front_page_modules_labels, $fieldpress_front_page_modules_descriptions, $fieldpress_front_page_modules_ordered;

		if ( !is_array( $fieldpress_front_page_modules ) ) {
			$fieldpress_front_page_modules = array();
		}

		if ( class_exists( $class_name ) ) {
			$class																 = new $class_name();
			$fieldpress_front_page_modules_labels[ $module_name ]				 = $class->label;
			$fieldpress_front_page_modules_descriptions[ $module_name ]		 = $class->description;
			$fieldpress_front_page_modules[ $section ][ $module_name ]			 = $class_name;
			$fieldpress_front_page_modules_ordered[ $section ][ $class->order ] = $class_name;
		} else {
			return false;
		}
	}
...
/**
 * flush_rewrite_rules() wrapper for FieldPress.
 *
