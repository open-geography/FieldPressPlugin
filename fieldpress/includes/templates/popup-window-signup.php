<form id="popup_signup_form">
	...
	<?php if ( cp_user_can_register() ) { ?>
		...
		<?php } ?>
		<input type="hidden" value="<?php esc_attr_e( isset( $_POST['field_id'] ) ? (int) $_POST['field_id'] : ' ' ); ?>" name="field_id"/>
	<?php
	} else {
	...
	<div class="cp_popup_buttons">
		...
		<label class="cp_popup_col_2">
			<a href="" class="cp_login_step" data-field-id="<?php esc_attr_e( isset( $_POST['field_id'] ) ? (int) $_POST['field_id'] : ' ' ); ?>"><?php _e( 'Already have an Account?', 'cp' ); ?></a>
		</label>

		<label class="cp_popup_col_2 second-child">
			<?php
			if ( cp_user_can_register() ) {
				$prereq = get_post_meta( $_POST['field_id'], 'prerequisite', true );
				if ( $prereq == 'false' ) {
					?>
					<button class="apply-button signup-data" data-field-id="<?php esc_attr_e( isset( $_POST['field_id'] ) ? (int) $_POST['field_id'] : ' ' ); ?>"><?php _e( 'Create Account', 'cp' ); ?></button>
				<?php
				}
			}
			?>
